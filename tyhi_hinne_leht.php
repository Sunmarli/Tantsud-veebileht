...
$kask_select=$yhendus->prepare(
    "SELECT paariID FROM tantsupaarid WHERE hinne1=0 ;");
$kask_select->bind_result($paariID);
$kask_select->execute();
...
$kask_update = $yhendus->prepare("UPDATE tantsupaarid SET hinne1 = ? WHERE paariID = ?");
    $kask_update->bind_param("ii", $selectedValue, $selected_id);
    $kask_update->execute();
    $kask_update->close();
$yhendus->close();
    header("Location: tyhi_hinne_leht2.php");
    exit();
}
